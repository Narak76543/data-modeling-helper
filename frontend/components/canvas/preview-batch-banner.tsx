...
export function PreviewBatchBanner({
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 z-40 w-full max-w-2xl px-4 select-none pointer-events-none">
      <div className="bg-surface/95 backdrop-blur border border-accent/40 shadow-lg rounded-[2px] p-3 pointer-events-auto flex items-center justify-between space-x-4">
        {/* Information & Title */}
        <div className="flex items-start space-x-2.5 min-w-0 flex-1">
          <div className="p-1.5 bg-accent/10 border border-accent/30 rounded-[2px] text-accent shrink-0 mt-0.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-[10px] font-mono font-medium px-1.5 py-0.2 bg-accent/10 text-accent border border-accent/20 rounded-[2px]">
                {tableCount} tables
              </span>
              {hasIssues ? (
                <span className="flex items-center space-x-1 text-[10px] font-mono text-warning bg-warning/10 px-1.5 py-0.2 border border-warning/20 rounded-[2px]">
                  <AlertTriangle className="w-3 h-3 shrink-0" />
                  <span>{validationResult?.summary.totalIssues} issue(s) flagged</span>
                </span>
              ) : (
                <span className="flex items-center space-x-1 text-[10px] font-mono text-success bg-success/10 px-1.5 py-0.2 border border-success/20 rounded-[2px]">
